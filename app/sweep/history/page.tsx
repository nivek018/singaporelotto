...
export const metadata = constructMetadata({
    ...
    description: "Browse past Singapore Sweep results with date range search",
    path: "/sweep/history",
});
...
async function getHistory(page: number = 1, limit: number = 20, fromDate?: string, toDate?: string) {
    ...
    try {
        let query = 'SELECT data, draw_date FROM results WHERE type = "Sweep"';
        const params: any[] = [];
        ...
        // Get total count
        let countQuery = 'SELECT COUNT(*) as count FROM results WHERE type = "Sweep"';
...
export default async function SweepHistoryPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
            ...
            <section className="relative overflow-hidden bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-700 dark:from-gray-800 dark:via-gray-900 dark:to-gray-800 py-16 px-4">
                ...
                <div className="max-w-4xl mx-auto text-center relative z-10">
                    <h1 className="text-4xl md:text-5xl font-bold text-white mb-4 tracking-tight">
                        Singapore Sweep History
                    </h1>
                    <p className="text-lg text-blue-100 dark:text-gray-300 max-w-2xl mx-auto leading-relaxed">
                        SINGAPORE SWEEP HISTORY – Access the list of previous <a href="/sweep" className="text-white underline hover:text-yellow-300">Singapore Sweep draw results</a>. View the Big Sweep winning numbers for every monthly draw held in the past. Also view <a href="/4d/history" className="text-white underline hover:text-yellow-300">4D history</a> and <a href="/toto/history" className="text-white underline hover:text-yellow-300">TOTO history</a>.
                    </p>
                </div>
            </section>
            ...
            <div className="max-w-6xl mx-auto px-4 py-8">
                                    ...
                                    return (
                                        <tr key={result.drawNo} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                            ...
                                            <td className="px-4 py-4 text-center">
                                                <span className="px-3 py-1.5 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 rounded-lg font-mono font-bold">
                                                    {result.winning[0]?.toString().padStart(7, '0')}
                                                </span>
                                            </td>
                                            <td className="px-4 py-4 text-center">
                                                <span className="px-3 py-1.5 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg font-mono font-bold">
                                                    {result.winning[1]?.toString().padStart(7, '0')}
                                                </span>
                                            </td>
                                            <td className="px-4 py-4 text-center">
                                                <span className="px-3 py-1.5 bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 rounded-lg font-mono font-bold">
                                                    {result.winning[2]?.toString().padStart(7, '0')}
                                                </span>
                                            </td>
                                            <td className="px-4 py-4 text-center">
                                                <a
                            ...
                            <div key={result.drawNo} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4">
                                ...
                                <div className="grid grid-cols-3 gap-2 text-center">
                                    <div>
                                        ...
                                        <span className="inline-block px-2 py-1 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 rounded font-mono font-bold text-xs">
                                            {result.winning[0]?.toString().padStart(7, '0')}
                                        </span>
                                    </div>
                                    <div>
                                        <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">2nd</div>
                                        <span className="inline-block px-2 py-1 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded font-mono font-bold text-xs">
                                            {result.winning[1]?.toString().padStart(7, '0')}
                                        </span>
                                    </div>
                                    <div>
                                        <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">3rd</div>
                                        <span className="inline-block px-2 py-1 bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 rounded font-mono font-bold text-xs">
                                            {result.winning[2]?.toString().padStart(7, '0')}
                                        </span>
                                    </div>
                                </div>
                ...
                {totalPages > 1 && (
                    <div className="flex justify-center items-center gap-4 mt-8">
                        {page > 1 && (
                            <a
                                href={`/sweep/history?page=${page - 1}${from ? `&from=${from}` : ''}${to ? `&to=${to}` : ''}`}
                ...
                <section className="mt-12 space-y-6">
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                        <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-4">About the Singapore Sweep Archive</h2>
                        <div className="space-y-3 text-gray-700 dark:text-gray-300 text-sm">
                            <p>
                                Browse recent Singapore Sweep draw results in this archive, including jackpot winners, top prize numbers, and 2D winning combinations. This archive grows with every monthly draw.
                            </p>
                            <p>
                                <strong className="text-gray-900 dark:text-white">Draw Schedule:</strong> Singapore Sweep is drawn once per month on the first Wednesday at 6:30 PM SGT. It is a pre-printed lottery with tickets ranging from 1000000 to 4499999.
                            </p>
                            <p>
                                <strong className="text-gray-900 dark:text-white">Prize Structure:</strong> Each draw offers over 3.5 million prizes, from the $2.3 million jackpot down to $6 for matching the last 2 digits.
                            </p>
                            <p>
                                <strong className="text-gray-900 dark:text-white">Ticket Validity:</strong> Winning tickets are valid for 180 days from the draw date. Check this archive for old monthly draws.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="bg-amber-50 dark:bg-amber-900/10 rounded-xl border border-amber-200 dark:border-amber-800 p-5">
                        <h3 className="font-semibold text-gray-900 dark:text-white mb-2 flex items-center gap-2">
                            <span className="text-amber-500">💡</span> Tips for Singapore Sweep Players
                        </h3>
                        <ul className="text-gray-700 dark:text-gray-300 text-sm space-y-2">
                            <li>• <strong>Monthly Draws:</strong> Since there is only one draw per month, keep your tickets safe until results are announced.</li>
                            <li>• <strong>Check 2D Numbers:</strong> Even if you did not win a main prize, check if your last 2 digits match the 2D winning numbers.</li>
                            <li>• <strong>Pre-printed Tickets:</strong> Unlike 4D, you cannot choose your own numbers. Buy early to ensure you get a ticket before they sell out.</li>
                        </ul>
                        <p className="text-xs text-gray-500 dark:text-gray-400 italic mt-3">
                            Disclaimer: Singapore Sweep is a pre-printed lottery game. Ticket availability depends on remaining unsold tickets at outlets.
                        </p>
                    </div>
                </section>
            </div>
