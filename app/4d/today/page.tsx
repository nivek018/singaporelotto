...
async function getResult(dateStr: string) {
    try {
        const [rows]: any = await pool.query(
            'SELECT data FROM results WHERE type = "4D" AND draw_date = ? LIMIT 1',
            [dateStr]
        );
        if (rows.length > 0) {
...
export default async function FourDTodayPage() {
            ...
            <div className="max-w-4xl mx-auto">
                ...
                {data ? (
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-12 text-center">
                        ...
                        <p className="text-sm text-gray-400 mt-4">Draws are usually held on Wednesday, Saturday, and Sunday at 6:30 PM.</p>
                    </div>
                )}
            </div>
        </main>
